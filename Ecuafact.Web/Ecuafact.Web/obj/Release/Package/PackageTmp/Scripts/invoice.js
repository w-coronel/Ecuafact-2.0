var Invoice = function () {
    ...
    var initialized = false,
        ...
        main = function () {
            ...
            $(".select-issuePointCode").on("change", set_issuePoint);
            ...
            configure_contrib();
            refresh_totals();
            configure_additionals_issuePoint();
        },
        ...
        set_establishment = function () {
            ...
            if (obj && obj.value.length > 0) {
                ...
                var est = Invoice.establishments.find(
        ...
        set_issuePoint = function () {
            var obj = $('.select-issuePointCode')[0];
            if (obj && obj.value.length > 0) {
                // Obtengo la informacion del punto de emisión
                var estabCode = $(".select-establishment").val();
                var est = Invoice.establishments.find(function (i) { return i.Code == estabCode; });
                var issuepoint = est.IssuePoint.find(function (i) { return i.Code == obj.value; });
                if (Object.entries(issuepoint).length > 0) {
                    if (issuepoint.CarrierRUC && issuepoint.Name) {
                        validate_additional(issuepoint, true);
                    }
                    else {
                        $("#adicional-items > tbody").empty();
                        configure_additionals();
                    }
                }
            }
        ...
        configure_additionals_issuePoint = function () {
            set_issuePoint();
        },
        ...
        add_additional = function (adname, advalue, hidden, carrier = 0) {
            additionalCount++;
            ...
            var markup =
                ...
                '  <input name="AdditionalFields[' + additionalCount + '].IsCarrier" value="' + carrier + '" type="hidden"></input>' +
                ...
                for (var i = 0; i <= additionalCount; i++) {
                    ...
                    if ($det.length > 0) {                        
                        ...
                        if ($nam.length > 0) {
                            if ($nam.val() === adname) {
                                if ($nam.length > 0) {
                                    if ($nam.val() === "PUNTO DE EMISIÓN TRANSPORTISTA") {
                                        $det.parents("tr").remove()
                                    }
                                    else if ($nam.val() === "RUC TRANSPORTISTA") {
                                        $det.parents("tr").remove()
                                    }
                                    else if ($nam.val() === "RAZON SOCIAL TRANSPORTISTA") {
                                        $det.parents("tr").remove()
                                    }
                                    else if ($nam.val() === "PLACA TRANSPORTISTA") {
                                        $det.parents("tr").remove()
                                    }
                                    else if ($nam.val() === "REGIMEN TRANSPORTISTA") {
                                        $det.parents("tr").remove()
                                    }
                                    else if ($nam.val() === "RESOLUCIÓN N° TRANSPORTISTA") {
                                        $det.parents("tr").remove()
                                    }
                                    else if ($nam.val() === "EMAIL TRANSPORTISTA") {
                                        $det.parents("tr").remove()
                                    }
                                    else if ($nam.val() === "TELEFONO TRANSPORTISTA") {
                                        $det.parents("tr").remove()
                                    }
        ...
        add_payment = function () {
            ...
            var $payment = Invoice.paymentTypes;
            if ($payment) {
        ...
        add_detail = function () {
            ...
            var markup =
                ...
                '<div class="col-md-4 col-sm-12 col-xs-12 input-group" data-title="Detalle1" class="numeric" style="padding-bottom:5px;width:100%"> ' +
                '<input id="detname2_' + itemsCount + '" name="Details[' + itemsCount + '].Name2"  class="form-control item-name2" style="text-align:left;background:none;font-size:11px;" placeholder="Nombre2" />' +
                '<input id="detval2_' + itemsCount + '" name="Details[' + itemsCount + '].Value2"  class="form-control item-value2" style="text-align:left;background:none;font-size:11px;" placeholder="Valor2" />' +
                '</div>' +

                '<div class="col-md-4 col-sm-12 col-xs-12 input-group" data-title="Detalle1" class="numeric" style="padding-bottom:5px;width:100%"> ' +
                '<input id="detname3_' + itemsCount + '" name="Details[' + itemsCount + '].Name3"  class="form-control item-name3" style="text-align:left;background:none;font-size:11px;" placeholder="Nombre3" />' +
                '<input id="detval3_' + itemsCount + '" name="Details[' + itemsCount + '].Value3"  class="form-control item-value3" style="text-align:left;background:none;font-size:11px;" placeholder="Valor3" />' +
            ...
            if (calculating == 0) {
                ...
                for (var i = 0; i < itemObj.length; i++) {
                    ...
                    if (typeof itemInput !== 'undefined' && itemInput.length > 0) {
                        ...
                        var ivaItem = Invoice.taxes.find(o => o.SriCode == ivaSelect.value);
        ...
        select_product = function (repo) {
            ...
            if (typeof repo.data !== 'undefined') {
                ...
                var ivaitem = Invoice.taxes.find(o => o.Id == repo.data.IvaRateId);
        ...
        configure_contrib = function () {
            var searchContributorUrl = Invoice.contributorsUrl + "/SearchContribAsync";
        ...
        validate_additional = function (data, issuePoint = false) {
            
            if (issuePoint) {
                var $carrierIssuePoint = 0;
                var $carrierRuc = 0;
                var $nameTransp = 0;
                var $carPlate = 0;
                var $carrieremail = 0;
                var $carrierphone = 0;
                var $carrierContributor = 0;
                ...
                if (additionalCount > 0)
                {
                    for (var i = 0; i <= additionalCount; i++) {
                        ...
                        if ($det.length > 0) {
                            ...
                            if ($nam.length > 0) {                                
                                if ($nam.val() === "PUNTO DE EMISIÓN TRANSPORTISTA") {
                                    $carrierIssuePoint = 1;
                                    $val.attr('value', data.Code);
                                }
                                else if ($nam.val() === "RUC TRANSPORTISTA") {
                                    $carrierRuc = 1;
                                    $val.attr('value', data.CarrierRUC);
                                }
                                else if ($nam.val() === "RAZON SOCIAL TRANSPORTISTA") {
                                    $val.attr('value', data.Name);
                                    $nameTransp = 1;
                                }
                                else if ($nam.val() === "PLACA TRANSPORTISTA") {
                                    $val.attr('value', data.CarPlate);
                                    $carPlate = 1;
                                }                                
                                else if ($nam.val() === "REGIMEN TRANSPORTISTA") {
                                    $val.attr('value', data.CarrierContribuyente);
                                    $carrierContributor = 1;
                                }
                                else if ($nam.val() === "RESOLUCIÓN N° TRANSPORTISTA") {
                                    $val.attr('value', data.CarrierResolutionNumber);
                                    $resolutionNumber = 1;
                                }
                                else if ($nam.val() === "EMAIL TRANSPORTISTA") {
                                    $val.attr('value', data.CarrierEmail);
                                    $carrieremail = 1;
                                }
                                else if ($nam.val() === "TELEFONO TRANSPORTISTA") {
                                    $val.attr('value', data.CarrierPhone);
                                    $carrierphone = 1;
                                }
                    ...
                    if ($carrierIssuePoint === 0)
                        add_additional("PUNTO DE EMISIÓN TRANSPORTISTA", data.Code, true, 1);
                    if ($carrierRuc === 0)
                        add_additional("RUC TRANSPORTISTA", data.CarrierRUC, true, 1);
                    if ($nameTransp === 0)
                        add_additional("RAZON SOCIAL TRANSPORTISTA", data.Name, true, 1);
                    if ($carPlate === 0)
                        add_additional("PLACA TRANSPORTISTA", data.CarPlate, true, 1);                   
                    if ($carrierContributor === 0) {
                        if (data.CarrierContribuyente != null)
                        {
                            add_additional("REGIMEN TRANSPORTISTA", data.CarrierContribuyente, true, 1);
                        }
                    }
                    if ($resolutionNumber === 0)
                    {
                        if (data.CarrierResolutionNumber != null)
                        {
                            add_additional("RESOLUCIÓN N° TRANSPORTISTA", data.CarrierResolutionNumber, true, 1);
                        }
                    }
                    if ($carrieremail === 0)
                        add_additional("EMAIL TRANSPORTISTA", data.CarrierEmail, true, 1);
                    if ($carrierphone === 0)
                        add_additional("TELEFONO TRANSPORTISTA", data.CarrierPhone, true, 1);

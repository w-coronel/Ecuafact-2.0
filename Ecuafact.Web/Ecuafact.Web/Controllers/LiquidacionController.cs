...
namespace Ecuafact.Web.Controllers
{
    ...
    public class LiquidacionController : DocumentControllerBase
    {
        protected override string SessionName => SessionInfo.LIQUIDACIONES_SESSION;

        #region Acciones del controlador

        protected override string DocumentType => "03";
        ...
        protected override object DocumentById(string id)
        {
            ...
            {
                model = ServicioLiquidacion.BuscarLiquidacionPorId(IssuerToken, id);
        ...
        public ActionResult Editar(string id = null)
        {
            ...
            {                
                var model = ServicioLiquidacion.BuscarLiquidacionPorId(IssuerToken, id ?? "0")?.ToRequest();
                if (model == null)
                {
        ...
        public ActionResult Nuevo()
        {
            if (SessionInfo.UserSession?.Subscription?.Status == SubscriptionStatusEnum.Activa)
            {
                var model = new SettlementRequestModel();
        ...
        [HttpPost]
        [HandleJsonException]
        public async System.Threading.Tasks.Task<JsonResult> SaveSettlementAsync(SettlementRequestModel model)
        {
            ...
            if (ModelState.IsValid && model != null)
            {
                ...
                var response = await ServicioLiquidacion.GuardarLiquidacionAsync(IssuerToken, model);
                var text = await response.Content.ReadAsStringAsync();
                if (response.IsSuccessStatusCode)
                {
                    ClearSession();
                    var doc = JsonConvert.DeserializeObject<SettlementModel>(text);
        ...
        {
            var document = ServicioLiquidacion.BuscarLiquidacionPorId(IssuerToken, id);
        ...
        public ActionResult PrintDocument(SettlementModel document, string reportType)
        {

            if (document != null)
            {
                SettlementReport report = new SettlementReport(Issuer, Server.MapPath("~/"));
                ...
                if (result != null)
                {
                    ...
                    if (!string.IsNullOrEmpty(document.AccessKey) && document.AccessKey != "-")
                    {
                        filename = $"LQ{document.AccessKey}.{result.Extension}";
                    }
                    else if (!string.IsNullOrEmpty(document.AuthorizationNumber) && document.AuthorizationNumber != "-")
                    {
                        filename = $"LQ{document.AuthorizationNumber}.{result.Extension}";
                    }
                    else
                    {
                        filename = $"LQ{document.SettlementInfo.IssuedOn.Replace("/", "")}{Issuer.RUC}{document.DocumentTypeCode}{document.DocumentNumber.Replace("-", "")}.{result.Extension}";
                    }

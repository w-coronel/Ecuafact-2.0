...
{
    public class InvoiceReport : ReportBase<InvoiceModel>
    {
        public InvoiceReport(IssuerDto issuer, string path)
            : base(issuer, path)
        {
        }
        protected override string ReportName => "RptFactura";
         
        protected override List<ReportDataSource> GetDataSources(InvoiceModel document)
        {
        ...
        public DataSet GetDataSet(InvoiceModel model)
        {
            var ds = new DataSet();
            ...
            dsFactura.Rows.Add(model.Id, 12, Issuer.EnvironmentType.GetValorCore(), Issuer.IssueType.GetValorCore(), Issuer.BussinesName.ToUpper(), Issuer.TradeName.ToUpper(), Issuer.RUC,
                ...
                model.ContributorIdentificationType, model.InvoiceInfo.ReferralGuide, model.ContributorName, model.ContributorIdentification,
                model.InvoiceInfo.Subtotal, model.InvoiceInfo.Tip, model.InvoiceInfo.Total, model.Currency, model.Status,
                model.InvoiceInfo.SubtotalVat, model.InvoiceInfo.SubtotalVatZero, model.InvoiceInfo.SubtotalNotSubject, model.InvoiceInfo.SubtotalExempt,
                ...
                model.InvoiceInfo.Total, model.AuthorizationNumber, 0M);

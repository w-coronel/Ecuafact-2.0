...
{
    public class PurchasesReport : ReportBase<ObjectQueryModel<PurchaseReportResponse>>
    {
        public PurchasesReport(IssuerDto issuer, string path)
               : base(issuer, path)
        {
        }

        protected override string ReportName => "PurchasesReport";

        protected override List<ReportDataSource> GetDataSources(ObjectQueryModel<PurchaseReportResponse> document)
        {
        ...
        public static byte[] GenerateExcel(ObjectQueryModel<PurchaseReportResponse> model)
        {
            var reportList = model.Data.Entity;

            using (var excelPkg = new ExcelPackage())
            {
                var Sheet = excelPkg.Workbook.Worksheets.Add("Compras");

                Sheet.Cells[1, 1].Value = "REPORTE DE COMPRAS";
                ...
                Sheet.Cells[5, 26].Value = "Codigo 103";
                ...
                Sheet.Cells[5, 42].Value = "Deducible";
                ...
                foreach (var item in reportList)
                {
                    ...
                    Sheet.Cells[row, 26].Value = item.Retention103TaxCode;
                    ...
                    Sheet.Cells[row, 42].Value = item.Deducible;
                ...
                var table = Sheet.Tables.Add(new ExcelAddressBase(5, 1, row - 1, 42), "Ventas");

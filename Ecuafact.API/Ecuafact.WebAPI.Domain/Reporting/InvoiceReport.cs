...
{
    public class InvoiceReport : ReportBase<Document>
    {
        public Issuer Issuer { get; set; }
        

        public InvoiceReport(Issuer issuer)
        {
            Issuer = issuer;           
        }
        ...
        protected override List<ReportDataSource> GetDataSources(Document document)
        {
            ...
            return new List<ReportDataSource> {
                ...
                new ReportDataSource("DsFacturaTransportista", dataSet.Tables["DsFacturaTransportista"])
            };
        }
        ...
        public DataSet GetDataSet(Document model)
        {
            ...
            var dsFacturaTransportista = ds.Tables.Add("DsFacturaTransportista");
            ...
            dsFactura.Columns.Add("Rimpe", typeof(System.String));
            ...
            dsFactura.Rows.Add(model.Id, 12, Issuer.EnvironmentType.GetCoreValue(), Issuer.IssueType.GetCoreValue(), Issuer.BussinesName.ToUpper(), Issuer.TradeName.ToUpper(),
                ...
                model.InvoiceInfo.Total, model.AuthorizationNumber, 0M,Issuer.IsRimpe ? (model.IssuedOn >= new DateTime(2022, 1, 1, 0, 0, 0) ? TextoInfoAdicionalEnum.IsRimpe.GetCoreValue() : "") : "",
            ...
            dsFacturaTransportista.Columns.Add("IdFacturaAdicional", typeof(System.Int64));
            dsFacturaTransportista.Columns.Add("Nombre", typeof(System.String));
            dsFacturaTransportista.Columns.Add("Valor", typeof(System.String));
            dsFacturaTransportista.Columns.Add("NumLinea", typeof(System.Int32));
            var t = 0;
            var transportista = model.AdditionalFields?.Where(s => s?.IsCarrier == 1).ToList();            
            transportista?.ForEach(item =>{
                var name = item.Name.Contains("TRANSPORTISTA") ? item.Name.Replace("TRANSPORTISTA", "").Trim() : item.Name;               
                dsFacturaTransportista.Rows.Add(t, name, item.Value, item.LineNumber);
                t++;
            });
